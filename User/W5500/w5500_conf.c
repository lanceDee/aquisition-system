...
/*定义默认IP信息*/
uint8 local_ip[4]  ={192,168,31,88};											/*定义W5500默认IP地址*/
...
/*定义远端IP信息*/
uint8  remote_ip[4]={192,168,31,51};											/*远端IP地址*/
...
{	
   ...
   /*复制定义的配置信息到配置结构体*/
	...
	memcpy(&(ConfigMsg.rport),&remote_port,2);
	if(ip_from==IP_FROM_DEFINE)	
		printf("Configuring W5500 from self-defined info\r\n");
	...
	if(ip_from==IP_FROM_EEPROM)
	{
		...
		if( *(EEPROM_MSG.mac)==0x00&& *(EEPROM_MSG.mac+1)==0x08&&*(EEPROM_MSG.mac+2)==0xdc)		
		{
			...
			memcpy(remote_ip,EEPROM_MSG.rip, 4);
			remote_port = EEPROM_MSG.rport;
		}
		else
		{
...
/*		if(dhcp_ok==1)
	...
	//任务初始化之前减少打印动作，在系统启动后通过命令查询w5500的相关信息
	...
	printf("Server Listening port: %d", remote_port);
}
...
/**
*@brief		重新配置并存储网络信息函数
*@param		sub子网掩码 ip gw网关
*@return	无
*/
void reset_w5500_ip(uint8 *ip, uint8 *gw, uint8 *sub, uint8 *rip, uint16 rport){
	setSIPR(ip);
	setGAR(gw);
	setSUBR(sub);
	...
	memcpy(&ConfigMsg.rport, &rport, 2);
...
/*lancedemo**********************************/
void write_server_config_to_eeprom(uint8 *rip, uint16 rport)
{
	uint16 dAddr=18;
	rip[4] = rport&0xff;
	rip[5] = (rport&0xff00) >> 8;
	ee_WriteBytes(rip, dAddr, 6);
}
